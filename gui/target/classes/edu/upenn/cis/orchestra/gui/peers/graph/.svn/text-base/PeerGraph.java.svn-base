...
//	private boolean _drawn = false;
	...
	/**
	 * 
	 * @param system We need the whole/complete system to know how to relate the peers together (using
	 *               mappings)
	 */
...
/*
	///TODO: Not really efficient, optimize memory...
	private void addPeers (List<Peer> peers)
	{
		List<DefaultGraphCell> cells = new ArrayList<DefaultGraphCell> ();
		
		// Create all peers cells and prepare peer indices for next step
		Map<Peer,Map<Schema,DefaultGraphCell>> peerCellMap = new HashMap<Peer,Map<Schema,DefaultGraphCell>> ();
		...
		// TODO: Cache the associated mappings for GUI??
		boolean[][] schemasMatrix = new boolean[allSchemas.size()][allSchemas.size()];
		for (Peer p : peers)
		{		
			for (ScMapping mapp : p.getMappings())
			{
				// List the schemas (indexes) in the head
				Set<Integer> headSchemas = new HashSet<Integer>(mapp.getMappingHead().size());
				for (ScMappingAtom atom : mapp.getMappingHead())
					headSchemas.add(allSchemas.indexOf(atom.getSchema()));
				// Complete the matrix with a link from each body schema to each head schema
				for (ScMappingAtom atom : mapp.getBody())
					for (Integer headSchema : headSchemas)						
						schemasMatrix[allSchemas.indexOf(atom.getSchema())][headSchema.intValue()]=true;
			}
		}
		// Once we have the matrix: create the edges
...
*/
	...
	/**
	 * Iterate through the peers and add edges between them according to the mappings.  
	 * If m x n mapping appears, or if multiple 1-1 mappings between the same nodes, then
	 * add a mapping node rather than just a labeled edge.
	 *  
	 * @param peers
	 */
	private void addPeersAndMappings (List<Peer> peers)
	{
		...
		for (Peer p : peers) {
			for (Mapping mapp : p.getMappings()) {
				// List the schemas (indexes) in the head
				List<GuiDefaultGraphCell> headSchemas = new ArrayList<GuiDefaultGraphCell>(mapp.getMappingHead().size());
				List<GuiDefaultGraphCell> bodySchemas = new ArrayList<GuiDefaultGraphCell>(mapp.getBody().size());
				...
				for (Atom atom : mapp.getMappingHead()) 
				{
					...
					{
						GuiDefaultGraphCell c = peerCellMap.get(p).get(atom.getSchema());
						headSchemas.add(c);
				...
				for (Atom atom : mapp.getBody()) {
					if (!schemas.contains(atom.getSchema()))
					{
						...
						if (p2 != null) {
							GuiDefaultGraphCell c = peerCellMap.get(p2).get(atom.getSchema());
							bodySchemas.add(c);
						}
						schemas.add (atom.getSchema());
					}
				...
				boolean needsNode = (headSchemas.size() != 1 || bodySchemas.size() != 1 || headSchemas.get(0) == bodySchemas.get(0));
				PeerMappingVertex cell = null;
				// See if we already have added an edge along this pair; if so we add a node
				if (!needsNode)
					for (Pair pair : oneToOnes) {
						if (pair.first == headSchemas.get(0) && pair.second == bodySchemas.get(0))
							needsNode = true;
					}
				if (!needsNode) {
					oneToOnes.add(new Pair(headSchemas.get(0), bodySchemas.get(0)));
					oneToOnes.add(new Pair(headSchemas.get(0), bodySchemas.get(0)));
				}
				...
				addEdge(p, headSchemas, bodySchemas, cell, i, cells, needsNode, mapp);
	...
	private void addEdge (Peer p, List<GuiDefaultGraphCell> headCells,
			List<GuiDefaultGraphCell> bodyCells, PeerMappingVertex mappingCell, int mid,
			List<GuiDefaultGraphObj> cells, boolean needsNode, Mapping map)
	{
		if (!needsNode) {
			// Create the edge
			PeerMappingEdge edge = new PeerMappingEdge (p, map, /*map.getMappingHead().get(0).getPeer().getId() +*/ map.getId());//new String("m" + Integer.valueOf(mid)));
	 ...
	 * @param p Peer to represent with a cell
	 */
	private void addPeer (Peer p, Schema s, 
							List<GuiDefaultGraphObj> cells, 
	 ...
	 * @param p Peer to represent with a cell
	 */
	private PeerMappingVertex addMappingNode (Peer p, Mapping map,
			int mapId)
	{
		// Create the cell
		PeerMappingVertex pCell = new PeerMappingVertex (p, map, /*map.getMappingHead().get(0).getPeer().getId() +*/ map.getId());//"m" + String.valueOf(mapId));
...
/*	
	private void addEdges (boolean[][] schemasMatrix,
								List<Schema> allSchemas,
								List<Peer> allSchemasPeers,
								Map<Peer,Map<Schema,DefaultGraphCell>> peerCellMap,
								List<DefaultGraphCell> cells)
	{
		for (int i = 0 ; i < schemasMatrix.length ; i++)
			for (int j = 0 ; j < schemasMatrix.length ; j++)
				if (i != j && schemasMatrix[i][j])
				{
					// Create the edge
					DefaultEdge edge = new DefaultEdge ();
					DefaultGraphCell src = peerCellMap.get(allSchemasPeers.get(i)).get(allSchemas.get(i));
					DefaultGraphCell target = peerCellMap.get(allSchemasPeers.get(j)).get(allSchemas.get(j));
					// Connect to default ports
...
*/
	...
	public void applyLayout ()
	{
		...
		JGraphOrganicLayout orgLayout = new JGraphOrganicLayout ();
		orgLayout.setOptimizeBorderLine(false);
		orgLayout.setEdgeLengthCostFactor(0.008);
		orgLayout.setBorderLineCostFactor(3);
		orgLayout.setNodeDistributionCostFactor(1100000);
		orgLayout.setMaxIterations(orgLayout.getMaxIterations()*2);

		JGraphFacade facade = new JGraphFacade (getGraphLayoutCache());
		
		applyLayout(facade, orgLayout, true, 2, new Point(0,0), true);
	}
