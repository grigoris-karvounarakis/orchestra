...
public class GuiRadialTreeLayout implements JGraphLayout {
	 ...
	 */
	protected transient Map<Object, TreeNode> nodes = new Hashtable<Object, TreeNode>();
	...
	private void layoutTree0(JGraphFacade graph, TreeNode node) {
		...
		List<TreeNode> parent = new ArrayList<TreeNode>(1);
		parent.add(node);
	 ...
	 */
	private void layoutTreeN(JGraphFacade graph, int level, List<TreeNode> nodes) {
		...
		// Recompute the radiusx at each step according to the cells size and 
		// their children size

		/**** Olivier: avoid overlapping cells due to large cells **/
		float maxRootCellWidth = 0;
		float maxChildCellWidth = 0;
		for (TreeNode root : nodes)
		{
			// ZI: skip cases where we have nulls
			if (root.getCell() == null || graph.getCellView(root.getCell()) == null ||
					((CellView) graph.getCellView(root.getCell())).getBounds() == null)
				continue;
			maxRootCellWidth = Math.max(maxRootCellWidth, (float)((CellView) graph.getCellView(root.getCell())).getBounds().getWidth());
			for (TreeNode child : root.getChildren())
				maxChildCellWidth = Math.max(maxChildCellWidth, (float)((CellView) graph.getCellView(child.getCell())).getBounds().getWidth());
		}
		double locRaduisX = Math.max(radiusx, (maxRootCellWidth) + (maxChildCellWidth) + 30);
		/**** End Olivier ***/
		
		Iterator<TreeNode> nitr = nodes.iterator();
		while (nitr.hasNext()) {
			...
			for (i = angleOffset; itr.hasNext(); i++) {
				...
				if (moveRoots || level > rootLevel) {
					// Olivier: use local radius
					node.x = rootx + ((level * locRaduisX) * Math.cos(node.angle));
