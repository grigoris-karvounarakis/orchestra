...
public class TransactionViewer extends JPanel {
	...
	public TransactionViewer (Peer p, OrchestraSystem sys)  throws DbException
	{
		...
		/*
		while (dbs.size() < numPeers) {
			if (! peers.hasNext()) {
				throw new RuntimeException("System must have at least " + numPeers + " peers");
			}
			String id = peers.next().getId();
			if (id.equals(p.getId())) {
				continue;
			}
			dbs.add(sys.getRecDb(id));
		}*/
...
//		Db db = dbs.get(dbI);
		...
		LegendGraph g = TransactionGraph.createLegend(600, 40, subPan.getBackground());
		...
		_tg.getSelectionModel().addGraphSelectionListener(new GraphSelectionListener ()
					{
						public void valueChanged(GraphSelectionEvent evt) 
						{
								...
								{
									TxnPeerID t = ((TransactionVertex) _tg.getSelectionCell()).getTxn();
									int prio = ((TransactionVertex) _tg.getSelectionCell()).getPriority();
		...
		_slider.setBorder(BorderFactory.createTitledBorder(p.getId() + " Reconciliation Epoch"));
		if (num > 3)
			    ...
			    new ActionListener() {
			        public void actionPerformed(ActionEvent e) {
			        	try {
							...
							JOptionPane.showMessageDialog(null, ex.getMessage(), "Error accessing DB", JOptionPane.ERROR_MESSAGE);
			        	}
			        }
			    }
			);
	...
	/*
	private List<TxnPeerID> setUpTrans(Db peerDB) throws ClassNotFoundException {
		ArrayList<TxnPeerID> trans = new ArrayList<TxnPeerID>();
		...
		try {
			...
			peer3.add(modN2N5);
			tid0 = dbs.get(0).addTransaction(peer0);
			dbs.get(0).publish();
			dbs.get(0).reconcile();
			dbs.get(1).reconcile();
			dbs.get(2).reconcile();
			tid1 = dbs.get(1).addTransaction(peer1);
			tid2 = dbs.get(2).addTransaction(peer2);
			dbs.get(1).publish();
			dbs.get(3).reconcile();
			dbs.get(2).publish();
			dbs.get(3).reconcile();
			tid3 = dbs.get(3).addTransaction(peer3);
			dbs.get(3).publish();
			dbs.get(0).reconcile();
	...
	*/
	...
	Schema s;
	RelationSchema rs;
	SchemaTuple tN1, tN2, tN3, tN4, tN5, tM4, tM5, tNull;
	...
	protected void setUp(Db curDb) throws Exception {
		s = curDb.getSchema();
		rs = s.getRelationSchema("R");

		tN1 = new SchemaTuple(rs);
		tN1.set("name", "Nick");
		tN1.set("val", 1);
		tN2 = new SchemaTuple(rs);
		tN2.set("name", "Nick");
		tN2.set("val", 2);
		tN3 = new SchemaTuple(rs);
		tN3.set("name", "Nick");
		tN3.set("val", 3);
		tN4 = new SchemaTuple(rs);
		tN4.set("name", "Nick");
		tN4.set("val", 4);
		tN5 = new SchemaTuple(rs);
		tN5.set("name", "Nick");
		tN5.set("val", 5);
		tM4 = new SchemaTuple(rs);
		tM4.set("name", "Mark");
		tM4.set("val", 4);
		tM5 = new SchemaTuple(rs);
		tM5.set("name", "Mark");
		tM5.set("val", 5);
		tNull = new SchemaTuple(rs);
		tNull.set("name", "Fred");
