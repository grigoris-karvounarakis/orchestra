...
public class TransactionGraph extends BasicGraph {
	...
	public void applyLayout ()
	{
		...
		JGraphFacade facade = new JGraphFacade (this, _notAntecedents.toArray());
		facade.tilt(_notAntecedents, 800 / 2, 600 / 2);

		JGraphHierarchicalLayout hLayout = new JGraphHierarchicalLayout();
		hLayout.setFixRoots(true);
		hLayout.setDeterministic(true);
		hLayout.setOrientation(SwingConstants.WEST);
		hLayout.setCompactLayout(true);
		hLayout.setInterHierarchySpacing(160);
		hLayout.setInterRankCellSpacing(180);
		...
		double delta = 0.0;
		delta = getWidth() - GraphLayoutCache.getBounds(getGraphLayoutCache().getCellViews()).getWidth();
		
		facade.translateCells(_cells, -delta, 10D);
		
		nested = facade.createNestedMap(true, false); 
		getGraphLayoutCache().edit(nested);
