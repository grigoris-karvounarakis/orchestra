...
public class ProvenanceGraph extends BasicGraph {
	...
	public void clearGraph() {
		getGraphLayoutCache().remove(_cells.toArray());
		_root = null;
		_cells.clear();
...
//								values.add(new AtomConst("NULL(" + + ")");
				...
				Rule inverseRule = new Rule(head, body, null, _system.getMappingDb());
	...
	{
		//TupleContext cont = new TupleContext(_root, _root.getOrigin());
		//Object[] root = {_tupleNodes.get(cont)};
		Object[] root = {_rootNode};
		//List<DefaultGraphCell> cells = new ArrayList<DefaultGraphCell>();
		//_cells.add(_tupleNodes.get(new TupleContext(cont)));
		JGraphFacade facade = new JGraphFacade (this, root);
		
		GuiRadialTreeLayout hLayout = new GuiRadialTreeLayout();
		
		applyLayout(facade, hLayout, true, new Point(0,0), true);
