...
public class BerkeleyDBStoreServer implements TransactionSource {
	...
	Map<AbstractPeerID,Schema> schemas = null;
	...
	Database lastRecno;
	// (peer, recno) -> epoch
	Database recnoEpochs;
	// epoch -> list of tids
	Database epochContents;
	// (peer, tid) -> decision (accepted[boolean], recno[int])
	Database decisions;
	// (peer, recno) -> tid (allows multiple values for each key)
	Database reconAcceptedTxns;
	// (peer, recno) -> tid (allows multiple values for each key)
	Database reconRejectedTxns;
	// tid -> transaction contents
	Database txns;
	
	SchemaIDBinding _mapStore;
	
	final int port;

	DatabaseConfig dc, reconTxnsDc;

//	File configFile;
	LinkedList<WorkerThread> workers = new LinkedList<WorkerThread>();
...
//		}
	...
	public void quit() throws IOException, InterruptedException, DatabaseException {
	...
	public static void main(String[] args) throws Exception {
		...
		System.out.println("Orchestra BerkeleyDB Store Server, Copyright (C) 2007 by Nicholas E. Taylor");
	...
	private class WorkerThread extends Thread {
		...
		public void run() {
			...
			try {
				for ( ; ; ) {
					try {
						...
						if (o instanceof EndOfStreamMsg) {
							...
							try {
								if (o instanceof SendTrustConditions) {
				...
				System.err.println("BerkleyDB Store Server caught exception: " + e);
				e.printStackTrace();
				exceptions.add(e);
			} finally {
				synchronized (workers) {
	...
	List<Exception> exceptions = Collections.synchronizedList(new LinkedList<Exception>());
