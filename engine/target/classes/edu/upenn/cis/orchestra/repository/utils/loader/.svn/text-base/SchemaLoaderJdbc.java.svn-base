...
public class SchemaLoaderJdbc implements ISchemaLoader {
	 ...
	 * 
	 * @param jdbcDriver
	 *            JDBC driver class to be used (make sure this is in the
	 *            classpath)
	 * @param jdbcURL
	 *            URL to use for the jdbc connection
	 * @param jdbcUserName
	 *            User name for the jdbc connection
	 * @param jdbcPassword
	 *            Password for the jdbc connection
	 */
	public SchemaLoaderJdbc(String jdbcDriver, String jdbcURL,
			String jdbcUserName, String jdbcPassword) {
		this(new DriverManagerDataSource(jdbcDriver, jdbcURL, jdbcUserName,
				jdbcPassword));
	}
	 ...
	 *             Thrown if an error occurs while reading database metadata
	 * @throws DuplicateSchemaIdException
	 *             if such an exception it thrown when building the
	...
	private class MetaLoadTables implements DatabaseMetaDataCallback {
		...
		public Object processMetaData(DatabaseMetaData dbmd)
			...
			for (TabName tableName : lstTab) {
				...
				try {
					Schema schema = null;
					String schemaName = (null == tableName._schemaName ? ""
							: tableName._schemaName);
					...
					} else if (null == _stringToSchema.get(schemaName)) {
		 ...
		 * 
		 * @param dataType
		 *            an SQL data type, as in the values specified in
	...
	private class MetaLoadConstraints implements DatabaseMetaDataCallback {
		 ...
		 * 
		 * @param fields
		 *            from which we're doing the removing.
		 */
