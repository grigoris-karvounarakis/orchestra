...
//		}
		allRules.addAll(getLocal2PeerRules());
		allRules.addAll(getSource2ProvRulesForIns());
		allRules.addAll(getProv2TargetRulesForIns());

		for (int i = 0; i < allRules.size(); i++) {
			Rule r = allRules.get(i);
			rules.add(compensationRule(r));
		}

//		for (int i = 0; i < getSource2ProvRulesForIns().size(); i++) {
//		Rule r = getSource2ProvRulesForIns().get(i);
//		rules.add(compensationRule(r, i));
//		}	
//		for (int i = 0; i < getProv2TargetRulesForIns().size(); i++) {
//		Rule r = getProv2TargetRulesForIns().get(i);
//		rules.add(compensationRule(r, i));
//		}
...
//		return new RecursiveDatalogProgram(rules, false);
	...
	protected Rule compensationRule(Rule rule) {
...
//			body.add(edbMinus);
	...
	protected List<Datalog> dredDeltaRules(){
		...
		List<Rule> s2p = new ArrayList<Rule>();
		List<Rule> p2t = new ArrayList<Rule>();

		for (int i = 0; i < getLocal2PeerRules().size(); i++) {
			Rule r = getLocal2PeerRules().get(i);
			l2p.addAll(dredDeltaRule(r));
		}
		dels.add(new NonRecursiveDatalogProgram(l2p));

		for (int i = 0; i < getSource2ProvRulesForIns().size(); i++) {
			Rule r = getSource2ProvRulesForIns().get(i);
			s2p.addAll(dredDeltaRule(r));
		}
//		for (int i = 0; i < getSource2ProvRules().size(); i++) {
//			Rule r = getSource2ProvRules().get(i);
//			s2p.addAll(dredDeltaRule(r));
//		}
		dels.add(new NonRecursiveDatalogProgram(s2p));

		for (int i = 0; i < getProv2TargetRulesForIns().size(); i++) {
			Rule r = getProv2TargetRulesForIns().get(i);
			p2t.addAll(dredDeltaRule(r));
		}

//		for (int i = 0; i < getProv2TargetRules().size(); i++) {
//			Rule r = getProv2TargetRules().get(i);
//			p2t.addAll(dredDeltaRule(r));
//		}
		dels.add(new NonRecursiveDatalogProgram(p2t));
		

//		for (int i = 0; i < getSource2ProvRules().size(); i++) {
//		Rule r = getSource2ProvRules().get(i);
//		dels.addAll(dredDeltaRule(r, i));
//		}
//		for (int i = 0; i < getProv2TargetRules().size(); i++) {
//		Rule r = getProv2TargetRules().get(i);
//		dels.addAll(dredDeltaRule(r, i));
//		}
	...
	protected List<Rule> dredDeltaRule(Rule rule) {
//		List<Atom> body = new ArrayList<Atom> (rule.getBody().size());
...
//		return provDeltas;  
		...
		List<Atom> body = new ArrayList<Atom> (rule.getBody().size());
		List<Rule> provenanceDels = new ArrayList<Rule>(body.size());


		Atom mapping_head = rule.getHead().deepCopy();
		mapping_head.setType(AtomType.DEL);

		for (Atom atom : rule.getBodyWithoutSkolems()){
			body.add(new Atom(atom, AtomType.NONE));
		}
		for (Atom atom : rule.getSkolemAtoms()){
			body.add(new Atom(atom, AtomType.NONE));
		}
		...
		for (int i = 0; i < rule.getBodyWithoutSkolems().size(); i++) {
			if(i > 0)
				body.set(i-1, new Atom(body.get(i-1), AtomType.NONE));
			body.set(i, new Atom(body.get(i), AtomType.DEL));

			Rule newRule = new Rule(mapping_head, body, rule.getParentMapping(), 
					true, getMappingDb());
			//				newRule.setReplaceValsWithNullValues();
			provenanceDels.add(newRule);
		}
