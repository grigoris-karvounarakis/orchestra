...
//			_pk = tab._pk;
	...
	throws InvalidBeanException, UnsupportedTypeException
	{
		...
		_hasLocalData = relBean.hasLocalData();
	}
...
//		greg: hack to add edb bit to peer relations
		...
		Relation rel;
		if (pkName != null) {
			rel = new Relation(dbcatalog, dbschema, dbtable, name, descr, mat, hasLocal, fields, pkName, pkFieldNames);
		} else {
			rel = new Relation(dbcatalog, dbschema, dbtable, name, descr, mat, hasLocal, fields);
		}
