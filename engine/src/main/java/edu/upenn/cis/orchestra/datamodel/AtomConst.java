...
public class AtomConst extends AtomArgument {
	...
	/**
	 * Creates a new atom constant value from its bean representation
	 * (Bean type field value must be "C")
	 * @param bean Bean to create from
	 */
	public AtomConst (ScMappingAtomValueBean bean)
	{		
		_value = bean.getId();
	}
	...
	/**
	 * Creates a bean representation of this constant value
	...
	public synchronized ScMappingAtomValueBean toBean() {
		...
		try
		{
			bean.setId(getType().getStringRep(getValue()));
