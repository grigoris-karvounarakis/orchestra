...
public class SchemaConverterStatementsGen {
	...
	public Map<Relation,List<String>> createTableConversionStatements (List<String> statements,
			...
			SqlDb db) throws MetaDataAccessException {
		...
		createDeltaTables (statements, withNoLogging, database, stratified, NO_LOGGING, db);
					...
					if (type != AtomType.NONE) {
						...
						for (int i = 0; i < rel.getFields().size(); i++) {
							...
							if (tablename.endsWith("_L_INS") || tablename.endsWith("_R_INS") ||
								...
								cols.add(new SqlColumnDef(rel.getField(i).getName() + RelationField.LABELED_NULL_EXT, 
										"INTEGER", "1"));
							else
								cols.add(new SqlColumnDef(rel.getField(i).getName() + RelationField.LABELED_NULL_EXT,
										"INTEGER", null));
					...
					//					(rel.getName().endsWith(Relation.REJECT) && (type != AtomType.NONE)))
				{
					...
					if(stratified && 
						...
						if(tablename.endsWith("_L_DEL") || tablename.endsWith("_R_DEL") || 
							...
							cols.add(new SqlColumnDef("STRATUM", "INTEGER", null));
						}
					}
					...
					if (type != AtomType.NONE) {
						...
						// gregkar
						if (!Config.getTempTables() 
								|| !(tablename.startsWith(SqlStatementGen.sessionSchema) // || type != AtomType.NONE
								)
						) {
					...
					Vector<ZColumnDef> v = new Vector<ZColumnDef>();
					//					boolean first = true;
					for (RelationField fld : rel.getFields()) {
						boolean fldInKey = rel.getPrimaryKey().getFields().contains(fld);
						if(fldInKey){
							v.add(new SqlColumnDef(fld.getName(), "", null));
						}
					}
					...
					if (!INDEX_ALL_FIELDS) {
						...
						//						TEMPORARY HACK
						v.add(new SqlColumnDef("KID", "", null));
						...
						if(rel.hasLabeledNulls()){
							for (RelationField fld : rel.getFields())
							{
								...
								v.add(new SqlColumnDef(fld.getName() + RelationField.LABELED_NULL_EXT, "", null));
							}
						}
	...
	private List<String> addLabeledNullColumns (Relation rel,
		...
		for (RelationField fld : rel.getFields())
		{
			...
			//			if(!fldInKey){
			if (!labNulls.contains(fld.getName() + RelationField.LABELED_NULL_EXT))
				cols.add(new SqlColumnDef(fld.getName() + RelationField.LABELED_NULL_EXT,
						"INTEGER", "1"));
			//			statements.add ("ALTER TABLE " + rel.getFullQualifiedDbId() 
