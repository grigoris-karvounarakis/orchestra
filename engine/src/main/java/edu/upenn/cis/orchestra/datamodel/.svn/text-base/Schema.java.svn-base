...
public class Schema implements Serializable, AbstractTuple.TupleFactory<Relation,Tuple> {
	...
	throws InvalidBeanException
	{
		...
		Map<ScRelationBean, Relation> oldNew = new HashMap<ScRelationBean, Relation> ();
		for (ScRelationBean relB : schemaBean.getRelations())
		{
			try
			{
				Relation rel = new Relation (relB);
				addRelation(rel);
				oldNew.put(relB, rel);
				...
				throw new InvalidBeanException (schemaBean, ut.getMessage());
			}
		}
		for (Map.Entry<ScRelationBean, Relation> entry : oldNew.entrySet())
	...
	public String getNameForID(int relationID) {
		if (relationID != Relation.NO_ID)
			return relationNames.get(IDToIndex.get(relationID));// - relOffset);
		else
			return "";
	}
	 ...
	 */
	public synchronized SchemaBean toBean ()
	{
