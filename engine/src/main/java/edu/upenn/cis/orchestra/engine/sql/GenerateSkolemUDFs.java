package edu.upenn.cis.orchestra.engine.sql;
 ...
 *
 */
public class GenerateSkolemUDFs {
	/**
...
//			ret.add("Dat", "DatVal" + Integer.toString(curDepth), "DATE", "Date");
	...
	static ArrayList<SkolemParameters> getAttribStatements(ArrayList<SkolemParameters> aList, int curDepth, int maxDepth) {
...
//			ret.add(getAttribDDL(a, AType.DAT, curDepth));
	...
	throws java.io.IOException {
		...
		for (int i = 0; i < s.getNumAttribs(); i++) {
			def.append(", ");
			String attr = s.getAttribNameAt(i);
			def.append(attr + ".trim()");
		}
		def.append(");\n");
		
		def.append("\t\t\tlog.println(\"Returned \" + skVal);\n");
		def.append("\t\t\tlog.println(\"for \" + skolemName  + \"(\" ");
				
		
		for (int i = 0; i < s.getNumAttribs(); i++) {
			String attr = s.getAttribNameAt(i);
			if(i < s.getNumAttribs()-1)
				def.append(" + " + attr + ".trim() + \", \"");
			else
				def.append(" + " + attr + ".trim()");
		}
