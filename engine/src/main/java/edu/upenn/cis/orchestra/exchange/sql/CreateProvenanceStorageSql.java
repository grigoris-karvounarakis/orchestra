...
//							MRULE added already
							toApply.addAll(d.createSQLTableCode(rel.getName(), 
									suffix, rel, false, true, rel.hasLabeledNulls(), false, withLogging));
...
//							Don't normalize attr names
							toApply.addAll(d.createSQLIndexCode(rel.getName(), suffix, rel, indexes, rel.hasLabeledNulls(), false, withLogging));
						}else{
//							Add MRULE
//							toApply.addAll(d.createSQLTableCode(rel.getName(), 
...
//							MRULE added already
							toApply.addAll(d.createSQLTableCode(rel.getName(), 
									suffix, rel, false, false, rel.hasLabeledNulls(), false, withLogging));
...
//	}
	...
	public void createProvenanceDbTable (final Relation rel, boolean withLogging, IDb db)
	{
		final List<String> toApply = new ArrayList<String>();
		SqlDb d = (SqlDb)db;
		List<RelationField> indexes =  rel.getPrimaryKey().getFields();
		...
		if (!relExists)
		{
			for (final Atom.AtomType type : Atom.AtomType.values()){
				...
				if(type != AtomType.RCH && (!Config.getStratified() || type != AtomType.ALLDEL)
					...
					if(Config.getStratified() &&
					   (type == AtomType.DEL || type == AtomType.INS)){
						toApply.addAll(d.createSQLTableCode(rel.getDbRelName(), 
								suffix, rel, false, true, rel.hasLabeledNulls(), false, withLogging));
						//toApply.addAll(createProvenanceTable ("M" + ruleIndice + suffix, 
						//		relLoc, buffDel, indexes, allVars.size()));
						toApply.addAll(d.createSQLIndexCode(rel.getName(), suffix, rel, indexes, rel.hasLabeledNulls(), false, withLogging));
