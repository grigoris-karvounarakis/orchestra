...
package edu.upenn.cis.orchestra.p2pqp.bdd;
...
public class JFactory extends BDDFactory {
    ...
    private class bdd extends BDD {
         ...
         */
        public synchronized BDD apply(BDD that, BDDOp opr) {
            int x = _index;
            int y = ((bdd) that)._index;
            int z = opr.id;
         ...
         * author:mengmeng
         */
        public BDD replaceVar(int oldvar, int newvar) {
        	int x = _index;
            return makeBDD(bdd_replace_newvar(x, oldvar, newvar));
        }
    ...
    static final int MINPATHHASH(int r) {
        return r;
    }
    ...
    synchronized int not_rec(int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int bdd_ite(int f, int g, int h) {
        int res;
        firstReorder = 1;
    ...
    synchronized int ite_rec(int f, int g, int h) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int bdd_replace(int r, bddPair pair) {
        int res;
        firstReorder = 1;
    ...
    synchronized int bdd_replace_newvar(int r, int oldvar, int newvar) {
    	int res;
        firstReorder = 1;
        ...
        int oldvar_real = bddvar2level[oldvar];
    
        int newvar_real = bdd_ithvar(newvar);
        
        replacelast = bddvar2level[oldvar];
        
        int id = update_pairsid();
        ...
        again : for (;;) {
            try {
                INITREF();
                replaceid = (id << 2) | CACHEID_REPLACE;
                if (firstReorder == 0)
                    bdd_disable_reorder();
                res = replace_var_rec(r, oldvar_real, newvar_real);
                if (firstReorder == 0)
                    bdd_enable_reorder();
            } catch (ReorderException x) {
                bdd_checkreorder();
    ...
    synchronized int replace_var_rec(int r, int oldlevel, int newvar) {
        BddCacheDataI entry;
        int res;
        ...
        PUSHREF(replace_var_rec(LOW(r), oldlevel, newvar));
        PUSHREF(replace_var_rec(HIGH(r), oldlevel, newvar));

        int nowlevel = LEVEL(bdd_ithvar(bddlevel2var[LEVEL(r)]));
        if (LEVEL(r) == oldlevel) {
        	nowlevel = LEVEL(newvar);
        };
        res =
            bdd_correctify(
                nowlevel,
    ...
    synchronized int replace_rec(int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int bdd_correctify(int level, int l, int r) {
        int res;
        ...
        if (level == LEVEL(l) || level == LEVEL(r)) {
        	System.out.println(level + " " + LEVEL(l) + " " + LEVEL(r));
    ...
    synchronized int bdd_apply(int l, int r, int op) {
        int res;
        firstReorder = 1;
    ...
    synchronized int apply_rec(int l, int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int and_rec(int l, int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int or_rec(int l, int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int relprod_rec(int l, int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int appquant_rec(int l, int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int appuni_rec(int l, int r, int var) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int unique_rec(int r, int q) {
        BddCacheDataI entry;
        int res;
        int LEVEL_r, LEVEL_q;
    ...
    synchronized int quant_rec(int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int constrain_rec(int f, int c) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int compose_rec(int f, int g) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int veccompose_rec(int f) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int restrict_rec(int r) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int simplify_rec(int f, int d) {
        BddCacheDataI entry;
        int res;
    ...
    synchronized int satone_rec(int r) {
        if (ISCONST(r))
            return r;
    ...
    synchronized int satoneset_rec(int r, int var) {
    ...
    synchronized int bdd_fullsatone(int r) {
        int res;
        int v;
    ...
    synchronized void INITREF() {
        bddrefstacktop = 0;
    }
    synchronized int PUSHREF(int a) {
    	if (bddrefstacktop > bddrefstack.length -2) {
    		return 0;
    	}
        bddrefstack[bddrefstacktop++] = a;
        return a;
    }
    synchronized int READREF(int a) {
    	if (bddrefstacktop < a) {
    		return 0;
    	}
        return bddrefstack[bddrefstacktop - a];
    }
    synchronized void POPREF(int a) {
    	if (bddrefstacktop < a) {
    		return;
    	}
        bddrefstacktop -= a;
    ...
    synchronized int bdd_nodecount(int r) {
        int[] num = new int[1];
    ...
    double bdd_minpath(int r) {
        CHECK(r);

        miscid = CACHEID_MINPATH;

        if (minpathcache == null) minpathcache = BddCacheD_init(cachesize);
        
        return bdd_minpath_rec(r) - 1.0;
    }
    
    double bdd_minpath_rec(int r) {
        BddCacheDataD entry;
        double minpath;
        ...
        entry = BddCache_lookupD(minpathcache, MINPATHHASH(r));
        if (entry.a == r && entry.c == miscid)
            return entry.dres;

        double left = bdd_minpath_rec(LOW(r));
        double right = bdd_minpath_rec(HIGH(r));
   ...
   synchronized void bdd_markcount(int i, int[] cou) {
    ...
    static final int CACHEID_MINPATH = 0x5;
    ...
    BddCache minpathcache; /* Cache for minpath results */
    int cacheratio;
    int satPolarity;
    int firstReorder;
    ...
    *************************************************************************/
     ...
     */
    synchronized public int duplicateVar(int var) {
        if (var < 0 || var >= bddvarnum) {
            bdd_error(BDD_VAR);
            return bddfalse;
        }
    ...
    synchronized int bdd_setvarnum(int num) {
        int bdv;
        ...
        if (bddrefstack != null)
            free(bddrefstack);
        bddrefstack = new int[num * 1000 + 1];
        bddrefstacktop = 0;
     ...
     * author: mengmeng
     */
    public BDD load(ByteBufferReader bbr) throws Exception {
        int result = bdd_load_bytes(bbr);
     ...
     * author: mengmeng
     */
    public void save(ByteBufferWriter bbw, BDD b) throws Exception {
    	int x = ((bdd) b)._index;
        bdd_save_bytes(bbw, x);
    }
    /* (non-Javadoc)
     ...
     * author: mengmeng
     */
     int bdd_load_bytes(ByteBufferReader bbr) throws Exception {
        int n, tmproot;
        int root;

        lh_nodenum = bbr.readInt();
        ...
        if (lh_nodenum == 0) {
        	System.out.println("lh_nodenum is zero!");
        	throw new IllegalArgumentException("lh_nodenum is zero!");
        }
        lh_table[lh_nodenum - 1].next = -1;
        lh_freepos = 0;

        tmproot = bdd_loaddata_bytes(bbr);
     ...
     * author: mengmeng
     */
    synchronized int bdd_loaddata_bytes(ByteBufferReader bbr) throws Exception {
        int key, var, low, high, root = 0, n;
        ...
        for (n = 0; n < lh_nodenum; n++) {
            ...
            if (low < 0 || high < 0 || var < 0) {
            	System.out.println("Load wrong! NodeCount:"+lh_nodenum+" key:" + key + " var:" + var + " low:" + low + " high:" + high);
            	return bdd_error(BDD_FORMAT);	
            }
     ...
     *  author: mengmeng
     */
    synchronized void bdd_save_bytes(ByteBufferWriter bbw, int r) throws Exception {
        int[] n = new int[1];
        ...
        if (n[0] == 0) {
        	System.out.println("Invalid Low(true or false):"+ (LOW(r) == INVALID_BDD ? 1: 0));
        	throw new RuntimeException("nodecount is zero!");
        }

        bbw.addToBuffer(n[0]);

        bdd_save_rec_bytes(bbw, r);
        bdd_unmark(r);
     ...
     * author: mengmeng
     */
    synchronized void bdd_save_rec_bytes(ByteBufferWriter bbw, int root) throws Exception {
        ...
        bdd_save_rec_bytes(bbw, LOW(root));
        bdd_save_rec_bytes(bbw, HIGH(root));

        if (bddlevel2var[LEVEL(root)] < 0 || LOW(root) < 0 || HIGH(root) < 0) {
            System.out.println("Saved wrong! key:" + root + " var:" + bddlevel2var[LEVEL(root)] + " low:" + LOW(root) + " high:" + HIGH(root));
            bdd_error(BDD_FORMAT);
            return;
        }
        bbw.addToBuffer(root);
        bbw.addToBuffer(bddlevel2var[LEVEL(root)]);
        bbw.addToBuffer(LOW(root));
        bbw.addToBuffer(HIGH(root));
