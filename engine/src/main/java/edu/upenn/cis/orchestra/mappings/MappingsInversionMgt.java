...
public class MappingsInversionMgt {
	 ...
	 */	
	public static List<Rule> inverseMappings (List<Mapping> mappings, IDb db)
	{
		List<Rule> res = new ArrayList<Rule> ();
		for (Mapping mapp : mappings)
			res.addAll (inverseMapping(mapp, db));
		return res;
	}
	 ...
	 */	 
	public static List<Rule> splitMappingHead(Mapping mapping, IDb db){
		List<Rule> res = new ArrayList<Rule>();
		...
		for(Atom a : mapping.getMappingHead()){
			...
			Rule sp = new Rule(a, stb, mapping, db);
	...
	public static List<Rule> splitMappingsHeads(List<Mapping> mappings, IDb db){
		List<Rule> res = new ArrayList<Rule>();
		for(Mapping mapping : mappings){
			res.addAll(splitMappingHead(mapping, db));
		}
		return res;
	}
	 ...
	 * @return List or rules resulting of inversion
	 */
	private static List<Rule> inverseMapping (Mapping mapping, IDb db){
		Mapping skolMapping = skolemizeMapping(mapping);
		
		return splitMappingHead(skolMapping, db);
	}
	
	private static List<Rule> OLDinverseMapping (Mapping mapping, IDb db)
	{
		int skolemIndex = 0;
		...
		try{
			...
			// Skolem already created. The key is the variable name for which it was 
			// created
//			Map<String, ScMappingAtomValSkolem> skolems = new HashMap<String, ScMappingAtomValSkolem> ();
			Map<String, Atom> skolemAtoms = new HashMap<String, Atom> ();

			// Set of common variables names
			...
			// Convert the body from a set of mapping atoms to a set of rule atoms
			List<Atom> newBody = new ArrayList<Atom> ();
			for (Atom atom : mapping.getBody())
				newBody.add(new Atom(atom));
			...
			{
				List<Atom> skolemsForHeadAtom = new ArrayList<Atom> ();
				...
				List<AtomArgument> values 
				= new ArrayList<AtomArgument> (headAtom.getValues().size());
				...
				for (AtomArgument val : headAtom.getValues())
				{
					if (val instanceof AtomVariable)
					{
						...
						if (commonVars.contains(var))
							values.add(var.deepCopy());
						else{ // Existential var found
//							Old: create Skolem "variable"
...
//							ScField to TableSchema
							Relation mappingRel = mapping.getBody().get(0).getRelation();		

							List<RelationField> mappingKey;

							if(mappingRel.getPrimaryKey() != null){
								mappingKey = mappingRel.getPrimaryKey().getFields();
							}else{
								mappingKey = mappingRel.getFields();
							}

//							List<ScField> fields = getFieldsForSkolemParams(mapping, params);
							List<AtomArgument> keyParams = getVarsForFields(mapping, mappingRel, mappingKey);
...
//							ScMappingAtom skolemAtom = createSkolemAtom(var, params, skolemAtoms, mapping);
							...
							skolemsForHeadAtom.add(skolemAtom);
							var.setSkolemDef(skolemAtom);
							values.add(var.deepCopy());
						}
					}
					else{ // WHAT IS THIS CASE?
						//TODO: Skolem can contain variables to be renamed!
						if (val instanceof AtomSkolem)
							values.add (renameUserSkolem ((AtomSkolem) val));
						else
							values.add (val);
					}
				}

				if (!db.isBuiltInAtom(headAtom)) {
					// A new inversed rule can now be created
					Atom newHeadAtom = new Atom (headAtom, values);

					//			res.add (new Rule (newHeadAtom, newBody));
					skolemsForHeadAtom.addAll(newBody);
					res.add (new Rule (newHeadAtom, skolemsForHeadAtom, mapping, db));
				}
			}
			return res;
		}catch(IncompatibleTypesException e){
			Debug.println("Inversion of mapping failed due to type error in the mapping");
			e.printStackTrace();
			return null;
		}
...
//						}
	 ...
	 * @return List or rules resulting of inversion
	 */	
	public static List<Rule> skolemizeRules (List<Rule> rules, IDb db)
	{
		List<Rule> res = new ArrayList<Rule> ();
		for (Rule r : rules)
			res.addAll(splitMappingHead(skolemizeMapping(r), db));
		return res;
	}
	 ...
	 * @return List or rules resulting of inversion
	 */	
	public static List<Mapping> skolemizeMappings (List<Mapping> mappings, IDb db)
	{
