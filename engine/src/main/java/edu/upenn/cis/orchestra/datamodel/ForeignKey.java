//Source file: D:\\PENN\\SHARQ\\API\\SCHEMAAPI\\src\\main\\java\\edu\\upenn\\cis\\orchestra\\repository\\model\\ScForeignKey.java
...
public class ForeignKey extends IntegrityConstraint implements Serializable 
{
   ...
   public ForeignKey(String fkName, AbstractRelation rel, String[] fields, AbstractRelation refRelation, String[] refFields, boolean sloppy)
	   ...
	   if (!(sloppy || fieldsSet.containsAll(refRelation.getPrimaryKey().getFields()))) {
		   throw new IllegalArgumentException("Foreign key must reference all elements of primary key in referenced relation");
	   }
	   _refFields = Collections.unmodifiableList(fieldsList);
   }
   
   /**
    * Create  a new foreign key from its bean representation
    * @param bean Bean to get the properties from
    * @param rel Relation created from the relation bean encapsulating the foreign
    * 			 key bean 
    * @param schema Schema created from the schema bean encapsulating the bean <code>rel</code>
    * 				Used to get the referenced relation, which must already exist
    * @throws InvalidBeanException If a field or relation does not exist
    */
   public ForeignKey (ScForeignKeyBean bean, Relation rel, Schema schema)
   					throws InvalidBeanException
   {
	   ...
	   Relation refRel; 
	   try{
		   refRel = schema.getRelation(bean.getRefRelation());
	   }catch(RelationNotFoundException e){
		   refRel = null;
	   }
   
	   if (refRel==null)
		   throw new InvalidBeanException (bean, "Relation " + bean.getRefRelation() 
				   								+ " referenced in " + bean.getName() + " does not exist");
	   _refRelation = refRel;
	   _refFields = new ArrayList<RelationField> ();
	   for (String fldName : bean.getRefFields())
	   {
		   RelationField refFld = _refRelation.getField(fldName);
		   if (refFld == null)
			   throw new InvalidBeanException (bean, "Field " + fldName + " referenced in " + bean.getName() + " does not exist");
		   _refFields.add( refFld );
	   }
   }
   ...
   /**
    * Creates a bean representation of this object. 
    * @return The bean (actual type is ScForeignKeyBean)
    * @see IntegrityConstraint#setBeanProperties(ScConstraintBean)
    * @see ForeignKey#setBeanProperties(ScConstraintBean)
    */
   public ScConstraintBean toBean ()
   {
	   ScForeignKeyBean bean = new ScForeignKeyBean ();
    ...
    * @see ForeignKey#toBean()
    * @see IntegrityConstraint#setBeanProperties(ScConstraintBean)
    */
   protected void setBeanProperties (ScForeignKeyBean bean)
   {
	   List<String> ignoreProperties = new ArrayList<String> (2);
	   ignoreProperties.add ("refFields");
	   ignoreProperties.add ("refRelation");
	   
	   super.setBeanProperties(bean, ignoreProperties);
	   
	   /** Copy simple properties, do not use BeanUtils.copyProperties to avoid issues with the parent class */
	   bean.setType("F");
	   bean.setRefRelation(getRefRelation().getName());

	   
	   List<String> refFields = new ArrayList<String> (getRefFields().size());
	   for (RelationField fld : getRefFields())
		   refFields.add (fld.getName());
	   bean.setRefFields(refFields);
   }
