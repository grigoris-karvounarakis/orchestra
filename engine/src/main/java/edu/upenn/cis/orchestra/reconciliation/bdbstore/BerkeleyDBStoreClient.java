...
public class BerkeleyDBStoreClient extends UpdateStore {
	...
	public static class Factory implements UpdateStore.Factory {
		...
		public UpdateStore getUpdateStore(AbstractPeerID pid, SchemaIDBinding sch, Schema s, TrustConditions tc) throws USException {
		...
		public USDump dumpUpdateStore(SchemaIDBinding binding, Schema schema) throws USException {
			try {
				Socket socket = new Socket(host.getAddress(), host.getPort());
				ObjectOutputStream oos = new ObjectOutputStream(socket.getOutputStream());
				ObjectInputStream ois = new ObjectInputStream(socket.getInputStream());
	...
	BerkeleyDBStoreClient(InetSocketAddress host, SchemaIDBinding sch, Schema s, TrustConditions tc) throws USException {
