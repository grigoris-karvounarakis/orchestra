...
public class OuterJoinUnfolder {
	...
	public static List<Rule> unfoldOuterJoins(List<Rule> inputRules, List<RelationContext> outerJoinRelations, 
			AtomType baseType, AtomType resType, IDb db){
		...
		if(outerJoinRelations.size() == 0){
			...
			for(RelationContext relCtx : outerJoinRelations){
				...
				List<Mapping> invOJMappings = ojRel.getInverseOuterJoinMappings(db);
	...
	public static List<Rule> unfoldOuterJoinMapping(Rule rule, Mapping def, AtomType baseType, AtomType resType){
		...
		for(int i = 0; i < completedMaps.size(); i++){
			...
			Rule unfoldedRule = new Rule(rule.getHead().deepCopy(), remainingTargetAtoms, rule.getParentMapping(), rule.getDb());
	...
	public static List<Rule> oldUnfoldOuterJoins(List<Rule> inputRules, List<RelationContext> outerJoinRelations, 
		...
		if(outerJoinRelations.size() == 0){
			...
			List<Rule> inverseOuterJoinRules = getInverseOuterJoinRules(baseType, outerJoinRelations, db);
	...
	/** @deprecated */
	protected static List<Rule> getInverseOuterJoinRules(AtomType type, List<RelationContext> outerJoinRelations, IDb db) {
		List<Rule> ret = new ArrayList<Rule>();
		for(RelationContext relCtx : outerJoinRelations){
			ProvenanceRelation provRel = (ProvenanceRelation)relCtx.getRelation();
			//			List<Rule> inv = provRel.invertOJMappings(getMappingDb());
			List<Rule> inv = provRel.getInverseOuterJoinRules(db);
			for(Rule r : inv){
	...
	/** @deprecated */
	protected static List<Rule> getInverseOuterUnionRules(AtomType type, List<RelationContext> outerUnionRelations, IDb db) {
		List<Rule> ret = new ArrayList<Rule>();
		for(RelationContext relCtx : outerUnionRelations){
			ProvenanceRelation provRel = (ProvenanceRelation)relCtx.getRelation();
			//			List<Rule> inv = provRel.invertOJMappings(getMappingDb());
			List<Rule> inv = provRel.getInverseOuterUnionRules(db);
			for(Rule r : inv){
				r.getHead().setType(type);
				for(Atom a : r.getBody()){
					a.setType(type);
