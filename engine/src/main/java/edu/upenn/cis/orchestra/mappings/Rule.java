...
import edu.upenn.cis.orchestra.datamodel.UntypedAtom;
...
{
	/** Maps a variables with a selection from the first relation in the
	 *  body associated with this variable... cf. buildVarMap
	 */
	...
	IRuleCodeGen _code;
	List<Integer> _preparedParams = new ArrayList<Integer>();

	IDb _db;


	public Rule (Atom head, Atom b, Mapping mapping, IDb db)
	{
		super(head.deepCopy(), b.deepCopy());
		_db = db;
		_code = new RuleSqlGen(this, db.getBuiltInSchemas());
		setParentMapping(mapping);
	}

	public Rule (Atom head, List<Atom> body, Mapping mapping, IDb db)
	{
		_code = new RuleSqlGen(this, db.getBuiltInSchemas());
	...
	public Rule (Atom head, List<Atom> body, Mapping mapping, boolean onlyKeyAndNulls, IDb db)
	{
		this (head, body, mapping, db);
		_code = new RuleSqlGen(this, db.getBuiltInSchemas());
	...
	public Rule fresh (Map<String,String> varsNameMap, boolean asNewObject)
	{
		...
		if (asNewObject)
		{
			Rule res =  new Rule (newHead, newBody, getParentMapping(), onlyKeyAndNulls(), _db);		
	...
	public Rule deepCopy(){
		...
		Rule res = new Rule(getHead().deepCopy(), newbody, getParentMapping(), onlyKeyAndNulls(), _db);
	...
	public List<Rule> invertForReachabilityTest(boolean invertEdbs, boolean invertOthers, List<RelationContext> edbs) {
		...
		return Rule.invertMappingForReachabilityTest(this, invertEdbs, invertOthers, edbs, true, getDb(), true);
	}
	...
	public List<Rule> invertForReachabilityTest(boolean invertEdbs, boolean invertOthers, List<RelationContext> edbs, boolean joinWithNew) {
		//		We always seem to want onlyKeyAndNulls to be true, so call it here accordingly
		return Rule.invertMappingForReachabilityTest(this, invertEdbs, invertOthers, edbs, true, getDb(), joinWithNew);
	}

	public static List<Rule> invertMappingForReachabilityTest(Mapping mapping, boolean invertEdbs, boolean invertOthers, 
			List<RelationContext> edbs, boolean onlyKeyAndNulls, IDb db, boolean joinWithNew) {
		List<Rule> res = new ArrayList<Rule> ();
		...
		for(Atom a : mapping.getBody()){
			if(!a.isNeg() && !a.isSkolem()){
				...
				if(!allFound){
					...
					Rule newRule = new Rule(newHead, newBody, mapping, onlyKeyAndNulls, db);
	...
	static public Rule parse(OrchestraSystem catalog, String rule, 
		...
		return new Rule(th, body, null, catalog.getMappingDb());
	}
			...
			if(arg1 instanceof AtomVariable){
				...
				if(arg2 instanceof AtomConst){
					...
					if(!c1.equals(c2)){
						if(contained.getRelation() instanceof ProvenanceRelation){
							ProvenanceRelation provRel = (ProvenanceRelation)contained.getRelation();
							if(ProvRelType.OUTER_JOIN.equals(provRel.getType()) || ProvRelType.LEFT_OUTER_JOIN.equals(provRel.getType()) 
									|| ProvRelType.RIGHT_OUTER_JOIN.equals(provRel.getType()) || ProvRelType.INNER_JOIN.equals(provRel.getType()) ){
								if(c1.equals("0"))
									return false;
							}else{
								Debug.println("check");
				...
				if(arg2 instanceof AtomConst){
					...
					if(!c1.equals(c2)){ // The following unfolding only makes sense for provenance relations
						if(atom1.getRelation() instanceof ProvenanceRelation){
							...
							if(ProvRelType.OUTER_JOIN.equals(provRel1.getType()) || ProvRelType.LEFT_OUTER_JOIN.equals(provRel1.getType()) 
									|| ProvRelType.RIGHT_OUTER_JOIN.equals(provRel1.getType()) || ProvRelType.INNER_JOIN.equals(provRel1.getType())){
