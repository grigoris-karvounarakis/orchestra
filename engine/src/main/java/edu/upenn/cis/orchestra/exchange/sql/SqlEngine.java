...
//				updateDb, 
	...
	public List<RelationContext> getAllTables(final OrchestraSystem syst){
		ArrayList<RelationContext> tables = new ArrayList<RelationContext>();
		tables.addAll(getState().getEdbs(_system));
		tables.addAll(getState().getIdbs(_system));
		tables.addAll(getState().getMappingRelations());
		tables.addAll(getState().getOuterJoinRelations());
...
//		getMappingDb().evaluateFromShell(str, dir, true);
	...
	public void compareBaseTablesWithCopies() throws IOException, ParseException {
...
//		finalize();
	...
	public void softReset() throws IOException, ParseException, Exception {
		if (!_mappingDb.isConnected())
			_mappingDb.connect();
		_mappingDb.resetCounters();
		clearAllTables();
...
//				throw new RuntimeException("Unable to evaluate update" + e.getStackTrace());
	...
	throws IOException, ParseException
	{
		...
		List<RelationContext> rels = getState().getIdbs(system);
...
//			System.out.println("TOTAL NUM OF TUPLES IN BASE AND MAPPING RELATIONS AFTER PREPARE NON-INCREMENTAL: " + countAllTables());
			...
			de.resetCounters();
		} else if (insFirst || Config.getDRED()) {
...
//					!Config.getAutocommit(), Config.getJDBCDriver(), Config.getStratified(), getMappingDb().getSqlTranslator().getLoggingMsg()));
					...
					map.putAll(statementsGen.createTableConversionStatements(statements, 
							!Config.getAutocommit(), Config.getJDBCDriver(), Config.getStratified(), 
							getMappingDb().getSqlTranslator().getLoggingMsg(), getMappingDb()));
				} catch (MetaDataAccessException e) {
					e.printStackTrace();
					throw new ParseException("Unable to access SQL");
				}
...
//		createProvenanceTables(getState().getMappingRelations());
		...
		createProvenanceTables(getState().getOuterJoinRelations());
